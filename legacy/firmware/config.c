...
#define OLD_STORAGE_SIZE(last_member)                                        \
  ...
  storage_unlock("", NULL);
  if (config.has_pin) {
    storage_change_pin("", config.pin, NULL, NULL);
  }
...
bool config_unlock(const char *pin) {
  char oldTiny = usbTiny(1);
  secbool ret = storage_unlock(pin, NULL);
  usbTiny(oldTiny);
  return sectrue == ret;
}
...
bool config_changePin(const char *old_pin, const char *new_pin) {
  char oldTiny = usbTiny(1);
  secbool ret = storage_change_pin(old_pin, new_pin, NULL, NULL);
  usbTiny(oldTiny);
...
bool config_changeWipeCode(const char *pin, const char *wipe_code) {
  char oldTiny = usbTiny(1);
  secbool ret = storage_change_wipe_code(pin, NULL, wipe_code);
  usbTiny(oldTiny);
  return sectrue == ret;
}
