...
static secbool storage_upgrade_unlocked(const char *pin,
...
static secbool set_wipe_code(const char *wipe_code) {
  size_t wipe_code_len = strlen(wipe_code);
  if (wipe_code_len == 0) {
    // This is to avoid having to check pin != "" when checking the wipe
    // code.
    wipe_code = (const char *)WIPE_CODE_EMPTY;
  ...
  hmac_sha256(salt, WIPE_CODE_SALT_SIZE, (const uint8_t *)wipe_code,
              wipe_code_len, tag);
  ...
  if (sectrue != norcow_update_bytes(WIPE_CODE_DATA_KEY, 0,
...
static secbool set_pin(const char *pin, const uint8_t *ext_salt) {
  size_t pin_len = strlen(pin);

  // Fail if the PIN is the same as the wipe code. Ignore during upgrade.
  if (norcow_active_version != 0 &&
      sectrue != is_not_wipe_code((const uint8_t *)pin, pin_len)) {
    return secfalse;
  }
  ...
  derive_kek((const uint8_t *)pin, pin_len, rand_salt, ext_salt, kek, keiv);
...
static void init_wiped_storage(void) {
  ...
  ensure(set_wipe_code(""), "set_wipe_code failed");

  ui_total = DERIVE_SECS;
  ui_rem = ui_total;
  ui_message = PROCESSING_MSG;
  ensure(set_pin("", NULL), "init_pin failed");
}
...
static secbool unlock(const char *pin, const uint8_t *ext_salt) {
  const char *unlock_pin = pin;
  size_t unlock_pin_len = strlen(pin);
  ...
  uint32_t upgrade_version = get_upgrade_version();
  if (upgrade_version == 1 || upgrade_version == 2) {
    ui_total += DERIVE_SECS;
    ui_rem += DERIVE_SECS;
    legacy_pin = pin_to_int(pin);
    unlock_pin = (const char *)&legacy_pin;
    unlock_pin_len = sizeof(legacy_pin);
  }

  // Now we can check for wipe code.
  ensure_not_wipe_code((const uint8_t *)unlock_pin, unlock_pin_len);
  ...
  uint8_t kek[SHA256_DIGEST_LENGTH] = {0};
  uint8_t keiv[SHA256_DIGEST_LENGTH] = {0};
  derive_kek((const uint8_t *)unlock_pin, unlock_pin_len,
             (const uint8_t *)rand_salt, ext_salt, kek, keiv);
  memzero(&legacy_pin, sizeof(legacy_pin));
  ...
  if (sectrue != storage_upgrade_unlocked(pin, ext_salt) ||
...
secbool storage_unlock(const char *pin, const uint8_t *ext_salt) {
...
secbool storage_change_pin(const char *oldpin, const char *newpin,
  ...
  ui_rem = ui_total;
  ui_message = (oldpin[0] != '\0' && newpin[0] == '\0') ? VERIFYING_PIN_MSG
                                                        : PROCESSING_MSG;
...
void storage_ensure_not_wipe_code(const char *pin) {
  size_t pin_len = strlen(pin);
  ...
  if (get_upgrade_version() <= 2) {
    legacy_pin = pin_to_int(pin);
    pin = (const char *)&legacy_pin;
    pin_len = sizeof(legacy_pin);
  }

  ensure_not_wipe_code((const uint8_t *)pin, pin_len);
  memzero(&legacy_pin, sizeof(legacy_pin));
}
...
secbool storage_change_wipe_code(const char *pin, const uint8_t *ext_salt,
                                 const char *wipe_code) {
  if (sectrue != initialized || pin == NULL || wipe_code == NULL ||
      (pin[0] != '\0' && strcmp(pin, wipe_code) == 0)) {
    return secfalse;
  }

  ui_total = DERIVE_SECS;
  ui_rem = ui_total;
  ui_message = (pin[0] != '\0' && wipe_code[0] == '\0') ? VERIFYING_PIN_MSG
                                                        : PROCESSING_MSG;
...
static uint32_t pin_to_int(const char *pin) {
  uint32_t val = 1;
  size_t i = 0;
  for (i = 0; i < V2_MAX_PIN_LEN && pin[i] != '\0'; ++i) {
    if (pin[i] < '0' || pin[i] > '9') {
      return 0;
    }
    val = 10 * val + pin[i] - '0';
...
static secbool storage_upgrade(void) {
  ...
  if (norcow_active_version == 0) {
    ...
    if (sectrue == norcow_get(V0_PIN_KEY, &val, &len)) {
      char *pin = int_to_pin(*(const uint32_t *)val);
      if (pin == NULL) {
        return secfalse;
      }
      set_pin(pin, NULL);
      memzero(pin, strlen(pin));
    } else {
      set_pin("", NULL);
    }
...
static secbool storage_upgrade_unlocked(const char *pin,
  ...
  if (version == 1 || version == 2) {
    if (sectrue != set_pin(pin, ext_salt)) {
      return secfalse;
    }
  }
  ...
  if (version == 2) {
    ...
    if (*(uint32_t *)wipe_code_data != 0) {
      ...
      ret = set_wipe_code(wipe_code);
      memzero(wipe_code, strlen(wipe_code));
    }
  }
